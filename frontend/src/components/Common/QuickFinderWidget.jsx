...
const QuickFinderWidget = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/products/quick-search`, {
        ratings: payload
      });
      setResults(res.data || { exactMatches: [], suggestions: [] });
    } catch (e) {
          ...
          <div className="bg-white w-full max-w-[600px] rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-[fadeIn_0.2s_ease-out]">
            ...
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-amber-50 to-orange-50/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-extrabold text-slate-800 m-0">Tìm kiếm mở rộng</h3>
                  <p className="text-[12px] text-slate-500 m-0 mt-0.5">Tìm điện thoại theo nhu cầu của bạn</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-6 bg-slate-50">
              {results === null ? (
                          ...
                          {spec.tiers.sort((a,b) => a.rank - b.rank).map(tier => {
                            ...
                            return (
                              <button
                                ...
                                className={`p-2 text-[12px] md:text-[13px] font-semibold rounded-lg border transition-all cursor-pointer flex flex-col items-center justify-center gap-1 ${
                                  isSelected ? activeClass : colorClass
                                }`}
                              >
                                <span className="text-[10px] opacity-80 font-normal">Bậc {tier.rank}</span>
                                <span>{tier.name}</span>
                              </button>
                            );
                          })}
                ...
                <div>
                  {!loading && (
                    <div className="mb-5 bg-blue-50/80 border border-blue-100 rounded-xl p-3.5">
                      <h4 className="text-[13px] font-bold text-blue-800 mb-2.5 flex items-center gap-1.5 m-0">
                        <Search size={15} /> Kết quả tìm kiếm cho:
                      </h4>
                      <div className="flex flex-wrap gap-2">
                        {specs.map(spec => {
                          const tierId = ratings[spec._id];
                          if (!tierId) return null;
                          const tier = spec.tiers.find(t => t._id === tierId);
                          if (!tier) return null;
                          return (
                            <span key={spec._id} className="bg-white border border-blue-200 text-blue-700 text-[12px] font-semibold px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1.5">
                              <span className="opacity-70 font-medium">{spec.name}:</span>
                              <span>Bậc {tier.rank} ({tier.name})</span>
                            </span>
                          );
                        })}
                      </div>
                  ...
                  {loading ? (
                    ...
                    <div className="space-y-5">
                      ...
                      {results.exactMatches?.length === 0 && results.suggestions?.length > 0 && (
                        <div className="bg-amber-50 border border-amber-200 text-amber-800 p-3 rounded-xl text-sm font-medium mb-2 flex items-start gap-2">
                          <Wand2 size={18} className="text-amber-500 mt-0.5 shrink-0" />
                          <span>Không có sản phẩm nào khớp 100%. Dưới đây là các sản phẩm gợi ý (cao hoặc thấp hơn 1 bậc).</span>
                        </div>
                      )}

                      {/* Lấy danh sách sản phẩm để hiển thị: ưu tiên exactMatches, nếu không có thì lấy suggestions */}
                      {(() => {
                        const hasExact = results.exactMatches?.length > 0;
                        const mainList = hasExact ? results.exactMatches : results.suggestions;
                        const topProduct = mainList[0];
                        const relatedProducts = mainList.slice(1);
                        const youMightLike = hasExact ? results.suggestions : [];
                        ...
                        return (
                          <>
                            ...
                            <div className="relative bg-gradient-to-b from-amber-50 to-white p-1 rounded-2xl border-2 border-amber-300 shadow-[0_8px_24px_rgba(245,158,11,0.15)]">
                              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-white text-[11px] font-black uppercase tracking-wider px-4 py-1 rounded-full shadow-md whitespace-nowrap">
                                {hasExact ? "Lựa Chọn Phù Hợp Nhất" : "Đề Xuất Tốt Nhất"}
                              </div>
                              <div className="bg-white rounded-xl p-4 mt-2 flex flex-col md:flex-row items-center gap-5">
                                <img 
                                  src={topProduct.colorImages?.[0]?.imageUrl || topProduct.images?.[0]} 
                                  alt={topProduct.name} 
                                  className="w-32 h-32 object-contain drop-shadow-md"
                                />
                                <div className="flex-1 text-center md:text-left">
                                  <h4 className="text-lg font-extrabold text-slate-800 mb-1">{topProduct.name}</h4>
                                  <p className="text-amber-600 font-black text-xl mb-3">
                                    {getMinPrice(topProduct).toLocaleString()}đ
                                  </p>
                                  <Link 
                            ...
                            {relatedProducts.length > 0 && (
                              <div>
                                <h4 className="text-sm font-bold text-slate-600 mb-3 pl-1 flex items-center gap-2">
                                  {hasExact ? "Các lựa chọn khớp 100% khác" : "Các lựa chọn gợi ý khác"}
                                </h4>
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                                          ...
                                          {prod._isExact === false && (
                                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5 ${prod._isBetter ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                                              {prod._isBetter ? '↑ Cao hơn' : '↓ Tiết kiệm hơn'}
                                            </span>
                                          )}
                                        </div>
                                      </div>
                            ...
                            {youMightLike?.length > 0 && (
                              <div className="pt-4 border-t border-slate-200 mt-2">
                                <h4 className="text-sm font-bold text-amber-600 mb-3 pl-1 flex items-center gap-2">
                                  <Sparkles size={16} /> Có thể bạn thích (Chênh lệch 1 bậc)
                                </h4>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  {youMightLike.map(prod => (
                                    <Link 
                                      key={prod._id} 
                                      to={`/product/${prod.slug}`}
                                      onClick={() => setIsOpen(false)}
                                      className="flex items-center gap-3 bg-gradient-to-r from-amber-50/50 to-orange-50/50 p-3 rounded-xl border border-amber-100 hover:border-amber-400 hover:shadow-md transition-all no-underline group"
                                    >
                                      <img 
                                        src={prod.colorImages?.[0]?.imageUrl || prod.images?.[0]} 
                                        alt={prod.name} 
