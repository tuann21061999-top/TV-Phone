    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                ...
                const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/ai/accessory-recommendations`, config);
    ...
    useEffect(() => {
        const updatePages = () => {
            if (carouselRef.current && recommendations.length > 0) {
                ...
                const itemWidth = window.innerWidth < 768 ? 150 : 280; // Chiều rộng ước tính
            ...
            <div className="relative flex items-center w-full">
                ...
                >
                    {recommendations.map((item, idx) => {
                        ...
                        return (
                            // THU NHỎ KÍCH THƯỚC giống AIRecommend
                            <div key={idx} className="snap-start w-[140px] xs:w-[150px] sm:w-[170px] md:w-auto md:min-w-[260px] md:max-w-[280px] shrink-0 rounded-xl bg-white flex flex-col shadow-[0_2px_8px_rgba(0,0,0,0.04)] md:shadow-none transition-transform hover:-translate-y-1">
