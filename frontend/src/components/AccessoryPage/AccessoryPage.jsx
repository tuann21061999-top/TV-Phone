    ...
    <div className="min-h-screen bg-[#f5f7fa] font-sans">
      ...
      <div className="w-full max-w-[1400px] mx-auto py-4 px-4 sm:py-8 sm:px-10">
        ...
        <nav className="flex items-center gap-2 text-[13px] md:text-sm text-slate-500 pb-3 md:pb-4">
          ...
          <span className="font-semibold text-slate-700">Phụ kiện Công nghệ</span>
        </nav>
        ...
        <div className="mb-4 md:mb-[30px] flex flex-col md:flex-row md:items-end justify-between gap-3 md:gap-0">
          <div>
            <h1 className="text-2xl md:text-[28px] font-bold text-slate-800 m-0 mb-1 md:mb-2">Phụ kiện Công nghệ</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-[30px] items-start">
          ...
          {/* SIDEBAR BỘ LỌC */}
          <aside className={`fixed top-0 left-0 h-full w-[280px] sm:w-[320px] bg-white z-[100] overflow-y-auto p-5 transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 lg:w-[260px] lg:h-auto lg:z-auto lg:p-5 lg:rounded-xl lg:border lg:border-slate-200 lg:shadow-sm lg:sticky lg:top-[100px] shrink-0 ${isFilterOpen ? "translate-x-0" : "-translate-x-full"}`}>
            
            {/* Header Sidebar Mobile */}
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 text-slate-800">
              <h3 className="flex items-center gap-2 text-[18px] font-bold text-gray-800 m-0">
            ...
            {/* CATEGORY */}
            <div className="mb-6">
              <h4 className="text-[14px] font-bold text-slate-800 mb-3 uppercase tracking-wide m-0">Danh mục</h4>
              {categories.map((cat) => (
                <button
                  key={cat.name}
                  className={`flex items-center gap-2 w-full p-2 border-none rounded-md cursor-pointer transition-colors mb-2 text-[14px] font-medium ${activeCategory === cat.name
            ...
            {/* PRICE */}
            <div className="mb-6">
              <h4 className="text-[14px] font-bold text-slate-800 mb-3 uppercase tracking-wide m-0">Khoảng giá</h4>
              <select
            ...
            {hasRatedProducts && (
              <div className="mb-6">
                <h4 className="text-[14px] font-bold text-slate-800 mb-3 uppercase tracking-wide m-0">Đánh giá</h4>
                <select
                  className="w-full p-2.5 border border-gray-200 rounded-md bg-white text-gray-700 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm cursor-pointer"
                  value={ratingFilter}
            ...
            {/* BRAND */}
            <div className="mb-6">
              <h4 className="text-[14px] font-bold text-slate-800 mb-3 uppercase tracking-wide m-0">Hãng</h4>
              <select
                className="w-full p-2.5 border border-gray-200 rounded-md bg-white text-gray-700 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm cursor-pointer"
                value={brandFilter}
            ...
            {/* SORT */}
            <div className="mb-6">
              <h4 className="text-[14px] font-bold text-slate-800 mb-3 uppercase tracking-wide m-0">Sắp xếp</h4>
              <select
                className="w-full p-2.5 border border-gray-200 rounded-md bg-white text-gray-700 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm cursor-pointer"
                value={sortOption}
          ...
          <section className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-4 lg:gap-5">
                {currentProducts.map((product) => {
                  ...
                  return (
                    <div
                      key={product._id}
                      className="bg-white p-2.5 sm:p-4 rounded-xl relative text-center flex flex-col h-full border border-slate-200 shadow-sm hover:shadow-[0_12px_24px_-8px_rgba(0,0,0,0.1)] hover:-translate-y-1 hover:border-slate-300 transition-all duration-300 group"
                    >
                      {/* Tags */}
                      <div className="absolute top-2 left-2 sm:top-3 sm:left-3 z-10 flex flex-col gap-1 items-start">
                        ...
                        {hasDiscount && discountPercent > 0 && (
                          <span className="bg-red-500 text-white text-[9px] sm:text-[11px] font-bold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded tracking-[0.5px]">
                            -{discountPercent}%
                          </span>
                        )}
                      </div>
                      ...
                      {/* Wishlist Button */}
                      <button
                        className={`absolute top-2 right-2 sm:top-2 sm:right-2 z-20 bg-white/90 border-none rounded-full w-7 h-7 sm:w-8 sm:h-8 p-0 flex items-center justify-center cursor-pointer shadow-sm transition-all duration-300 md:hover:scale-[1.15] hover:bg-white hover:shadow-[0_3px_10px_rgba(0,0,0,0.15)] ${favoriteIds.has(product._id) ? "bg-red-50 animate-heartPop" : ""
                          }`}
            ...
            {!loading && filteredProducts.length === 0 && (
              <div className="bg-white p-10 rounded-xl text-center text-gray-500 shadow-sm border border-dashed border-gray-200 mt-2">
      ...
      <GlobalArticle pageCode="accessories" />
      <Footer />
    </div>
  );
}
